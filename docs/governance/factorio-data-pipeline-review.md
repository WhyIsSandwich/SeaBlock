...
- Entry point: `scripts/orchestrate-factorio-processing.js`
- Extraction step runs Factorio dump commands:
  - `--dump-data`
  - `--dump-prototype-locale`
  - `--dump-icon-sprites`
- The orchestrator then invokes `scripts/process-factorio-data.js` with `--script-output`.
...
- Produces JSON and sprite assets in `docs/public/data/`.
...
```mermaid
flowchart LR
  factorioDump[FactorioScriptOutput] --> orchestrator[OrchestratorScript]
  orchestrator --> processor[ProcessFactorioData]
  processor --> artifacts[DocsPublicDataArtifacts]
...
- Orchestrator currently calls processing without passing `--locale`, so output defaults to English unless run manually.

## Findings and Risk Prioritization

### P0 Correctness

1. Runtime organized data key mismatch risk in `src/composables/useFactorioData.js`.
   - Data is organized by singular base types (`recipe`, `technology`), while utility methods reference plural keys (`recipes`, `technologies`).
   - Impact: empty results from helper methods and inconsistent behavior.
...
2. Monolithic pipeline/orchestration files.
   - `scripts/process-factorio-data.js` and `scripts/orchestrate-factorio-processing.js` are large and cross-cutting, increasing regression surface.

## Prioritized Improvement Matrix

| Priority | Improvement | Impact | Effort | Target files |
| --- | --- | --- | --- | --- |
| P0 | Normalize organized-data key usage and helper lookups | High | Small | `src/composables/useFactorioData.js` |
...
| P2 | Modularize pipeline/orchestration scripts by concern | Medium | Large | `scripts/process-factorio-data.js`, `scripts/orchestrate-factorio-processing.js` |
| P2 | Add artifact schema contract checks in CI/local validation | Medium | Medium | `scripts/process-factorio-data.js`, `docs/public/data/*.json`, test/validation scripts |

## Improvement Backlog

### Immediate Hardening (Now)

- Fix organized-data key consistency in `src/composables/useFactorioData.js`.
...
- Split `scripts/process-factorio-data.js` by concerns:
  ...
  - artifact writers
- Split `scripts/orchestrate-factorio-processing.js` into command modules and shared path/config utilities.
